...
object Injection {
    ...
    fun provideMusicPlayItemRepository(application: Application) = PlayItemRepository.getInstance(MusicPlayer.getInstance(Singleton.getInstance(AWMediaPlayer::class.java)), MediaDatabase.getInstance(application).musicDao() as BaseDao<Any>).apply {
        this.observeCurrentItem()
    }
